...
summary:    Apollo（阿波罗）是携程框架部门研发的配置管理中心，能够集中化管理应用不同环境、不同集群的配置，配置修改后能够实时推送到应用端，并且具备规范的权限、流程治理等特性。本文主要对Apollo的核心概念和使用方法做了着重介绍。
categories:
---
...
Apollo（阿波罗）是携程框架部门研发的配置管理中心，能够集中化管理应用不同环境、不同集群的配置，配置修改后能够实时推送到应用端，并且具备规范的权限、流程治理等特性。
